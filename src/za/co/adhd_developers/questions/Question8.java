package za.co.adhd_developers.questions;

import za.co.adhd_developers.tools.Question;

import java.util.ArrayList;

/**
 * Created by Grant on 2017/07/28.
 */
public class Question8 implements Question
{
    int limit = 13;
    long answer = 0;

    String theNumber = "";

    @Override
    public void doWork()
    {
        int strIndexStart = 0;

        while (strIndexStart <= theNumber.length()-limit)
        {
            long possibleAnswer = 1;
            ArrayList<Long> digits = new ArrayList<>();

            String tmpStr = theNumber.substring(strIndexStart, strIndexStart+limit);

            String[] numbers = tmpStr.split("|");

            for (String num : numbers)
            {
                Long tmpNumber = Long.parseLong(num);
                digits.add(tmpNumber);
            }

            for (int i = 0; i < digits.size(); i++)
            {
                possibleAnswer = possibleAnswer * digits.get(i);
            }

            if (this.answer < possibleAnswer)
            {
                this.answer = possibleAnswer;
            }

            strIndexStart++;
        }
    }

    @Override
    public void printAnswer()
    {
        System.out.println("=================================");
        System.out.println("Question 8");
//        System.out.println("=================================");
//        System.out.println(this.d1);
//        System.out.println(this.d2);
        System.out.println("The thirteen adjacent digits product is: " + this.answer);
        System.out.println("=================================");
    }
}
